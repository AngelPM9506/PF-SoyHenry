...
const Contact = () => {
  ...
  const validateInput = (input: contact) => {
    ...
    if (JSON.stringify(error) === '{}') { setDisable(false) }
    return error;
  }
  ...
  return (
    <Layout>
      ...
      <Center display={'flex'} flexDirection={'column'} margin={'1rem'}>
        ...
        <Text textAlign={'center'} margin={0}>If you have any questions or you need asistance you can write to us here, we will contact you as soon as possible.</Text>
      </Center>
              ...
              <Grid marginBottom={'10px'} h={'max-content'} w={['95vw', '95vw', '85vw', '45vw']}
                ...
                <GridItem borderRadius={'2xl'} padding={['10px', '10px', '20px']} rowSpan={5} colSpan={5} bg={'blackAlpha.100'}>
                  ...
                  <Flex justifyContent={'center'}>
                    <Button onClick={setChangeInputs} disabled={disable}
                    mt={4} type={'submit'} bg="highlight" color="primary">Send Message</Button>
                  </Flex>
                </GridItem>
              </Grid>
            </Center>
