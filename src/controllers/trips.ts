...
const TripsControllers = {
    ...
    */
    ...
    putTrip: async (body: body, query: query) => {
        ...
        if (!active && !description && !traveler) throw new Error('Missing data try again');
        
        let createUsers: createUsers[] = (idPartaker && Array.isArray(idPartaker)) ? idPartaker.map((idP: string) => {
            return {
                user: {
                    connect: {
                        id: idP.toString()
