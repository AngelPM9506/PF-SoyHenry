...
export default function ChatRoom(props: Props) {
    ...
    const sendMessage = (room: string, message: string, user: string, avatar: string) => {
    ...
    const renderAvatar = (avatar: string) => {
        return (<Image
            boxSize={'70px'}
            src={avatar || 'https://res.cloudinary.com/mauro4202214/image/upload/v1663331567/world-travelers/favicon.ico_c8ryjz.png'}
            alt={''}
            borderRadius={'full'}
    ...
    return (
        <Layout>
            <Heading as={'h2'} width={'100%'} textAlign={'center'} marginTop={'1rem'}>Chat: {trip.name}</Heading>
            <Box bg={useColorModeValue("RGBA(75,100,124,0.41)", "RGBA(75,100,124,0.41)")}
                width={['95%', '95%', '85%', '75%']} margin={'1rem auto'} padding={'0.1rem 1rem'} borderRadius={'3xl'}>
                {/**render chats */}
                <Box display={'flex'} flexDirection={'column'} scrollSnapAlign={'end'} width={['95%', '95%', '85%', '75%']} margin={'1rem auto'} height={'60vh'}
                    borderRadius={'2xl'} padding={'1rem'} overflow={'scroll'}
                    sx={{ '::-webkit-scrollbar': { display: 'none' } }} >
                    {chat && chat.map((m, i) => {
                        return (
                            <Box key={i} display={'flex'} alignItems={'center'}
                                flexDirection={userDb && (m.nameUser === userDb.data.name) ? 'row-reverse' : 'row'} >
                                <Flex borderRadius={'3xl'}
                                    bg={useColorModeValue("RGBA(255,255,255,1)", "rgba(209,223,227, 0.2)")}
                                    boxShadow={'0px 0px 13px 10px rgba(0,0,0,0.1)'}
                                    border={userDb && (m.nameUser === userDb.data.name)
                                        ? '1px solid #F3B46F' : '1px solid #02b1b1'}
                                    margin={'0.3rem'}
                                    alignItems={'center'}
                                    padding={'0.5rem 1rem'} maxWidth={'85%'} >
                                    {userDb && (m.nameUser === userDb.data.name) ? '' : renderAvatar(m.avatar)}
                                    <Flex margin={'0 0.2rem'} flexDirection='column'>
                                        <Text
                                            color={userDb && (m.nameUser === userDb.data.name) ? '#F3B46F' : '#02b1b1'}
                                            margin={'0.3rem 0.3rem 0 0.3rem'} fontStyle={'italic'} fontWeight={'bold'}>{m.nameUser}:</Text>
                                        <Text
                ...
                <Stack as={'form'} width={['95%', '95%', '85%', '75%']} margin={'0 auto 3rem auto'} onSubmit={putMessage}>
                    <FormControl display={'flex'} alignItems={'center'}>
                        <Textarea value={message} onChange={setInput} margin={'0.5rem auto'} resize={'none'} borderRadius={'3xl'}
